...
namespace WebAPI.Controllers
{
    ...
    public class ItemsController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<Item>>> GetAsync([FromQuery] string? title,
            [FromQuery] string? manufacture, [FromQuery] string? description, [FromQuery] double? price, [FromQuery] int? stock)
        {
            try
            {
                SearchItemParametersDto parameters = new(title, manufacture, description, price, stock);
